...
try {
  ...
  for (const [n, who] of [['Lash', 'Albertt'], ['Mina', 'lordnm'], ['Kelvin', 'Yndio']]) {
    await pickHero(n);
  ...
  await page.goto('http://localhost:4173/');
  await page.waitForFunction(() => document.querySelector('.app-header h1')?.textContent.startsWith('Infernus') && document.querySelectorAll('.tiles .tile').length >= 1, null, { timeout: 15000 });
  await page.click('.hero-picker-trigger');
  await page.waitForSelector('.hero-sheet .hero-filter');
  ...
  await page.reload();
  await page.waitForFunction(() => document.querySelector('.app-header h1')?.textContent.startsWith('Lash') && document.querySelectorAll('.tiles .tile').length >= 1, null, { timeout: 15000 });
  ...
  // item 3: clicking the second style tab on Warden changes the build and adds style= to the URL
  {
    ...
    if (tabs2.length >= 2) {
      const before = await page.evaluate(() => ({ tile: document.querySelector('.tiles .tile')?.textContent, heading: document.querySelector('.board h2')?.textContent }));
      await tabs2[1].click();
      await page.waitForFunction((prev) => { const t = document.querySelector('.tiles .tile')?.textContent; const h = document.querySelector('.board h2')?.textContent; return t !== prev.tile || h !== prev.heading; }, before, { timeout: 15000 });
